 ...
 * verdade; se não estiver e o sistema estiver em Demonstração, usa o
 * simulado; se não estiver e o sistema estiver em Produção, recusa subir.
 *
 * Esse último caso e o que importa. Subir em produção com gateway simulado
 * significaria dar assinatura de graca para quem clicasse, e o erro só
...
public class FornecedoresExternos {
    ...
    public EntregaDeVideo entregaDeVideo(ConfiguracaoDaOutorga configuracao, Clock relogio) {
        ...
        if (video != null && video.configurado()) {
            log.info("Entrega de vídeo: bucket {} em {}", video.bucket(), video.endpoint());
        ...
        log.warn("Entrega de vídeo em modo demonstração: todo play devolve o HLS de amostra.");
    ...
    public GatewayDePagamento gatewayDePagamento(ConfiguracaoDaOutorga configuracao,
        ...
        if (cobranca != null && cobranca.configurado()) {
            log.info("Cobrança pelo gateway em {}", cobranca.urlBase());
        ...
        log.warn("Cobrança em modo demonstração: nenhum dinheiro e movimentado.");
